...
#include "coupledTotalLagIncrBeam.H"
#include "fvm.H"
#include "fvc.H"
...
defineTypeNameAndDebug(coupledTotalLagIncrBeam, 0);
addToRunTimeSelectionTable(beamModel, coupledTotalLagIncrBeam, dictionary);


// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

coupledTotalLagIncrBeam::coupledTotalLagIncrBeam
(
    Time& runTime,
    const word& region
)
    ...
    )
{
    ...
    // Calculate initial strain vectors
    if (true)
    {
        ...
        gamma_ = ((refLambda_ - II) & i_);
        
        // Q_ = -(Lambda_ & (CQ_ & ((II - refLambda_) & i_)));
        // M_ = -(Lambda_ & (CM_ &  (refT.T() & fvc::snGrad(refTheta_))));
    }
...
scalar coupledTotalLagIncrBeam::evolve()
{
    ...
    do
    {
        ...
        do
        {        
            #include "coupledDWDThetaEqn_TLI.H"
        ...
        if (contactActive())
        {
            ...
            DW_.storePrevIter();
            Info << max(mag(DW_.internalField())) << endl;
    ...
    if (contactActive())
    {
        ...
        const_cast<volVectorField&>(DW_.prevIter()) =
            dimensionedVector("0", DW_.dimensions(), vector::zero);
        // Info << max(mag(DW_.prevIter().internalField())) << endl;
    }

    return initialResidual;
}

void coupledTotalLagIncrBeam::updateTotalFields()
{
    W_ = W_.oldTime() + DW_;
    ...
    // DW_ == dimensionedVector("0", DW_.dimensions(), vector::zero);
    // DW_.storePrevIter();    
    // DTheta_ == dimensionedVector("0", DTheta_.dimensions(), vector::zero);

    // DWDTheta_ ==
    //     dimensioned<vector6>
    //     (
    //         "0",
    //         DWDTheta_.dimensions(),
    //         vector6::zero
    //     );
}

tmp<vectorField> coupledTotalLagIncrBeam::currentBeamPoints(const label bI) const
{
    const fvMesh& mesh = this->mesh();
    ...
    else
    {
        ...
        const surfaceVectorField Wf =
            fvc::interpolate(W_.oldTime())
          + fvc::interpolate(DW_);
        // const surfaceVectorField Wf = fvc::interpolate(W_.oldTime() + DW_);
        surfaceVectorField curCf = mesh.Cf() + Wf;
        const vectorField& curCfI = curCf.internalField();
...
tmp<vectorField> coupledTotalLagIncrBeam::currentDisplacementIncrement() const
{
    label nPoints = this->mesh().nCells() + 1;
...
tmp<tensorField> coupledTotalLagIncrBeam::currentRotationIncrement() const
{
    label nPoints = this->mesh().nCells() + 1;
