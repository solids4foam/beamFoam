...
)
{
    ...
    //   if (bI == 1)
    //   {
	//   Info << "bI " << bI << " segI " << segI << " zeta " << zeta << endl;
	//   Info << "segI " << segI << " nbI " << nbI << " neiSegI " << neiSegI << " neiZeta " << neiZeta << endl;
    //   }
    
    //   Info << "LineContact.C::updateForce() start " << endl;
    ...
    //   Info << "LineContact.C::updateForce() lower upper beam segments and weights set " << endl;
	...
	// This section of the code does not work with
	// createMultipleCircularBeamMesh because
	// for this utility crossSections is not defined at all
	...
	//   Info << "explicit line contact force calculation start " << endl;
	
	//- Explicit line contact force per unit length contribution
	if (augmentedLagrangian)
	{
		...
		if (normalModel.epsilon()*gap < normalContactForceOffset())
		{
	...
	else
	{
	    ...
	    //   normalContactForce_ = lineWeight*
		//   normalModel.contactForce
		    //   (
			//   gap,
			//   normalContactForceOffset(),
	...
	*/	
	...
	//- SB: Delta n contribution
	vector neiDRdZeta =
	    splines[nbI].paramFirstDerivative(neiSegI, neiZeta);  
	vector neiDR2dZeta2 = splines[nbI].paramSecondDerivative(neiSegI,neiZeta);

	tensor ImNN = tensor::I - (n*n);
	
	tensor TT = (neiDRdZeta*neiDRdZeta)/
		    (
			(neiDRdZeta & neiDRdZeta)
		    //   - (n & neiDR2dZeta2)
		    );
	//   Info << "1st term: " << (neiDRdZeta & neiDRdZeta) << "\n" << " 2nd term: " << (n & neiDR2dZeta2) << endl;
	
	normalContactForceDerivative() +=
	(
	    mag(normalContactForce_)*(ImNN - (ImNN & TT))
	   /dist
	);
	...
	// this find Nearest point is creating a problem mostly	
	labelScalar np =
	...
	//   Info << "oldSegI: " << oldSegI << " oldZeta: " << oldZeta << endl; 
	//   Info << "oldNeiSegI: " << oldNeiSegI << " oldNiiZeta: " << oldNeiZeta << endl;
	
	//   Info << "oldAxialGap: " << oldAxialGap_ << endl;
	...
	//   Info << "trialAxialContactForce: " << trialAxialContactForce << endl;

        if (mag(trialAxialContactForce) > maxFrictionForce)
        {
	    //   Info << "Line contact slip case " << endl;
            axialContactForce_ =
        ...
        //   {
	    //   Info << "else  if loop line contact " << endl;
            //   axialContactForce_ =
                //   maxFrictionForce*T*sign(trialElasticAxialGap);
        ...
        {
	    //   Info << "line contact stick case" << endl;
    ...
    // SB - Port to ESI version    
    tmp<tensorField> tSegLambda =
        beam.beamPointData(Lambda, bI, segI);

    const tensorField& segLambda = tSegLambda();
    ...
    // SB: Port to ESI version
    tmp<tensorField> tNeiSegLambda =
           beam.beamPointData(Lambda, nbI, neiSegI);
    const tensorField& neiSegLambda = tNeiSegLambda();
