...
) const
{
    ...
    if (g <= forceOffset/epsilon_)
    {
        fc = forceOffset + fBar - epsilon_*g;
    }
    else if (g > forceOffset/epsilon_ && g <= (forceOffset/epsilon_ + gBar_))
    {
        //   fc = (epsilon_*gBar_ - fBar)*sqr(g/gBar_)
          //   - epsilon_*g + fBar;
	scalar a = epsilon_/(2*gBar_);
	
	scalar b = -(forceOffset/gBar_+ epsilon_);
	
	scalar c = fBar + sqr(forceOffset)/(2*gBar_*epsilon_) + forceOffset;
