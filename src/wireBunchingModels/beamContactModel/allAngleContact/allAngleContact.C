...
Foam::allAngleContact::allAngleContact()
:
    firstBeam_(-1),
    firstBeamSegment_(-1),
...
{
    // Info << "Inside set() of allAngleContact.C" << endl;
...
void Foam::allAngleContact::updateForce
(
...
)
{
	...
	//   Info << "contact angle allAngleContact.C file " 
	     //   << ::acos(mag(tan0 & tan1))*180.0/M_PI 
	     //   << endl;
		
	if
	(
	     contactAngle_ < upperContactAngleLimit 
	  && contactAngle_ > lowerContactAngleLimit  
	)
	{
	    // Info << "Inside contact angle < contact angle limit loop" << endl;
	    ...
	    scalar gap = dist - beam.R(bI) - beam.R(nbI);
            ...
            if (!Pstream::parRun())
            {
		// Info << "Line 299 of allAngleContact.C file" << endl;
                tensor curRM =
...
Foam::scalar Foam::allAngleContact::allAnglePointContactWeight
(
    const beamModel& beam,
    const PtrList<HermiteSpline>& splines,
    const scalar lowerContactAngleLimit,
...
//   formulation (adopted from Meier's thesis, eqn. 4.62, pg 185)
Foam::scalar Foam::allAngleContact::allAngleLineContactWeight
(
    const beamModel& beam,
    const PtrList<HermiteSpline>& splines,
    const scalar lowerContactAngleLimit,
...
// comment SB: This member function is probably not being used
Foam::scalar Foam::allAngleContact::calcNormalGap
(
    const beamModel& beam,
    const PtrList<HermiteSpline>& splines
) const
