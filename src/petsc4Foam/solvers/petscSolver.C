...
Foam::lduSolverPerformance Foam::petscSolver::scalarSolve
(
    scalarField& psi,
...
) const
{
    ...
    // const fvMesh& fvm = refCast<const fvMesh>(matrix_.mesh());
...
Foam::lduSolverPerformance Foam::petscSolver::solve
(
    scalarField& psi_s,
    const scalarField& source,
    const direction cmpt
...
) const
{
    ...
    if (f)
    {
        ...
        labelList globalCells(globalNumbering_.localSize());
        forAll(globalCells, cI)
        {
            globalCells[cI] = globalNumbering_.toGlobal(cI);
        }
        ...
        forAll(interfaces, patchi)
        {
            ...
            // if (intf)
            {
                if (isA<ggiLduInterface>(interfaces[patchi]))
                // if (isA<ggiLduInterface>(*intf))
                // if (isA<cyclicAMILduInterface>(*intf))
                {
                    FatalErrorInFunction
                        << "GGI is not supported" << nl
                        << exit(FatalError);
                }
        ...
        label off = globalNumbering_.offset(Pstream::myProcNo());
    ...
    else
    {
        ...
        forAll(interfaces, patchi)
        {
            ...
            // if (intf)
            {
                ...
                else if (isA<cyclicLduInterface>(interfaces[patchi]))
...
) const
{
    ...
    if (f)
    {
        ...
        if (nProcValues)
        {
            ...
            forAll(interfaces, patchi)
            {
                ...
                if (interfaces.set(patchi))
                {
                    ...
                    const scalarField& bCoeffs = coupleBouCoeffs()[patchi];
        ...
        labelList globalCells(globalNumbering_.localSize());
        forAll(globalCells, locCellI)
        {
            globalCells[locCellI] = globalNumbering_.toGlobal(locCellI);
        }
        ...
        // Connections to neighbouring processors
        {
            ...
            forAll(interfaces, patchi)
            {
                if (interfaces.set(patchi))
                {
                    ...
                    const label off =
                        globalNumbering_.offset(Pstream::myProcNo());
