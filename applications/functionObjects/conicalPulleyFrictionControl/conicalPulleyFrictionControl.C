...
#include "conicalPulleyFrictionControl.H"
...
namespace Foam
{
    defineTypeNameAndDebug(conicalPulleyFrictionControl, 0);

    addToRunTimeSelectionTable
    (
        functionObject,
        conicalPulleyFrictionControl,
        dictionary
    );
}
...
// bool Foam::conicalPulleyFrictionControl::writeData()
// {
...
Foam::conicalPulleyFrictionControl::conicalPulleyFrictionControl
(
    const word& name,
    const Time& t,
...
bool Foam::conicalPulleyFrictionControl::start()
{
    // return writeData();
    return true;
}
...
#if FOAMEXTEND > 40
bool Foam::conicalPulleyFrictionControl::execute(const bool forceWrite)
#else
bool Foam::conicalPulleyFrictionControl::execute()
#endif
{
    const fvMesh& mesh =
        time_.lookupObject<fvMesh>(regionName_);
...
bool Foam::conicalPulleyFrictionControl::read(const dictionary& dict)
{
    if (dict.found("region"))
    {
        dict.lookup("region") >> regionName_;
