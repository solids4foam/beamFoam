...
int main(int argc, char *argv[])
{
    ...
    argList::validArgs.append("distanceFromPulley");
...
#   include "createTime.H"
    ...
    // Read total path length
    scalar distanceFromPulley
    (
        readScalar(IStringStream(args.additionalArgs()[1])())
    );
    ...
    //- Add starting point
    {
        vector axis2 =
        (
            toroidalPulleys[0].axis()
           ^toroidalPulleys[0].polarAxis()
        );

        vector T0 =
            toroidalPulleys[0].origin()
          + (toroidalPulleys[0].minRadius() + distanceFromPulley)*axis2;

        if
        (
            (
                (axis2^toroidalPulleys[0].polarAxis())
              & toroidalPulleys[0].axis()
            )*toroidalPulleys[0].rotDir()
          < 0
        )
        {
            T0 = toroidalPulleys[0].origin()
              - (toroidalPulleys[0].minRadius() + distanceFromPulley)*axis2;
        }

        vector tangent0 = toroidalPulleys[0].polarAxis();
    ...
    //- Add remining points
    for (label i=1; i < toroidalPulleys.size(); i++)
    {
        if
        (
            mag
            (
                toroidalPulleys[i-1].axis()
              & toroidalPulleys[i].axis()
            )
          > (1-SMALL)
        )
        {
            ...
            (
                "cs",
                toroidalPulleys[i-1].origin(),
                toroidalPulleys[i-1].axis(),
                toroidalPulleys[i-1].polarAxis()
            );

            vector C0 =
                cs.localPosition(toroidalPulleys[i-1].origin());
            vector C1 =
                cs.localPosition(toroidalPulleys[i].origin());
            ...
            scalar r0 = (toroidalPulleys[i-1].minRadius() + distanceFromPulley);
            scalar r1 = (toroidalPulleys[i].minRadius() + distanceFromPulley);
           ...
           /(sqr(xP-c)+sqr(yP-d)) + d;
            ...
            )
            {
                ...
                {
                    vector gC0 = toroidalPulleys[i-1].origin();
        ...
        else // Add transition path segment
        {
            // Add last point of the previous pulley
            {
                ...
                vector axis2 =
                (
                    toroidalPulleys[lastPulley].axis()
                   ^toroidalPulleys[lastPulley].polarAxis()
                );

                vector Te =
                (
                    toroidalPulleys[lastPulley].minRadius()
                  + distanceFromPulley
                )*axis2;
                ...
                (
                    (
                        (
                            (
                                points[lastPoint]
                              - toroidalPulleys[lastPulley].origin()
                            ) ^ Te
                        )
                      & toroidalPulleys[lastPulley].axis()
                    )*toroidalPulleys[lastPulley].rotDir()
                  < 0
                )
                {
                    Te *= -1;
                ...
                Te += toroidalPulleys[lastPulley].origin();
                vector tangente = toroidalPulleys[lastPulley].polarAxis();

                // Add arc central point and tangent
                {
                    vector gC0 = toroidalPulleys[lastPulley].origin();
                    scalar r0 =
                        toroidalPulleys[lastPulley].minRadius()
                      + distanceFromPulley;
                    label li = points.size()-1;
            ...
            // Add straight transition path segment
            {
                ...
                (
                    toroidalPulleys[i].axis()
                   ^toroidalPulleys[i].polarAxis()
                );

                vector T0 =
                    toroidalPulleys[i].origin()
                  + (
                        toroidalPulleys[i].minRadius()
                      + distanceFromPulley
                    )*axis2;
                ...
                (
                    (
                        (axis2^toroidalPulleys[i].polarAxis())
                      & toroidalPulleys[i].axis()
                    )*toroidalPulleys[i].rotDir()
                  < 0
                )
                {
                    T0 = toroidalPulleys[i].origin()
                      - (toroidalPulleys[i].minRadius() + distanceFromPulley)
                       *axis2;
                }

                vector tangent0 = toroidalPulleys[i].polarAxis();
    ...
    //- Add last point
    {
        label lastPulley = toroidalPulleys.size()-1;
        ...
        (
            (
                ((points[lastPoint] - toroidalPulleys[lastPulley].origin())^Te)
              & toroidalPulleys[lastPulley].axis()
            )*toroidalPulleys[lastPulley].rotDir()
          < 0
        )
    ...
    //Add straight part of the path at the outlet
    {
        ...
        vector DL =  dl*toroidalPulleys[lastPulley].polarAxis();
    ...
    // Write path data to file
    {
        ...
        fileName pathFileName("toroidalPulleysPath");
        OFstream pathFile(toroidalPulleysDir/pathFileName);
    ...
    // Write path data to vtk file
    {
        ...
        fileName vtkFileName("toroidalPulleysPath.vtk");
        
        OFstream vtkFile(toroidalPulleysDir/vtkFileName);
