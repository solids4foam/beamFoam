...
bool Foam::beamDisplacements::writeData()
{
    if (patchFound_)
    {
        ...
        if (mesh.foundObject<volVectorField>("totW"))
        {
            const vectorField& D =
...
Foam::beamDisplacements::beamDisplacements
(
    const word& name,
    const Time& t,
    ...
    historyFilePtr_(NULL)
{
    ...
    else
    {
        WarningIn(this->name() + " function object constructor")
            << "beamDisplacements: historyPatch not specified" << endl;
    }
    ...
    if (historyFilePtr_.empty() && patchFound_)
    {
        ...
        if (Pstream::master())
        {
            ...
            (
                new OFstream
                (
                    historyDir/"beamDisplacements"+historyPatchName+".dat"
                )
            );
...
bool Foam::beamDisplacements::start()
{
    return writeData();
}


bool Foam::beamDisplacements::execute()
{
    return writeData();
}


bool Foam::beamDisplacements::read(const dictionary& dict)
{
    return true;
}
