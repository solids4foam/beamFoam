...
Description
    Calculate initial position and shape of two beams
...
#   include "createMesh.H"
    ...
    forAll(refWfI, faceI)
    {
        ...
        if (I == 1)
        {
            refWfI[faceI] = (newCfI[faceI] + DR1 - CfI[faceI]);
	...
	if (I == 2)
        {
            refWfI[faceI] = (newCfI[faceI] + DR2 - CfI[faceI]);

            refLambdaI[faceI] = tensor::I;
    ...
    forAll(refRMI, cellI)
    {
        ...
        if (I == 1)
        {
            refWI[cellI] = (newCI[cellI] + DR1 - CI[cellI]);
            refRMI[cellI] = Ry;
        }
	if (I == 2)
        {
            refWfI[cellI] = (newCfI[cellI] + DR2 - CfI[cellI]);
	    refRMI[cellI] = tensor::I;
        }
    }
    ...
    forAll(refWf.boundaryField(), patchI)
    {
        ...
        forAll(pRefWf, faceI)
        {
            ...
            if (I == 1)
            {
                pRefWf[faceI] = (newpCf[faceI] + DR1 - pCf[faceI]);
	    ...
	    if (I == 2)
            {
                pRefWf[faceI] = (newpCf[faceI] + DR2 - pCf[faceI]);
                pRefW[faceI] = pRefWf[faceI];

                pRefLambda[faceI] = tensor::I;
                pRefRM[faceI] = tensor::I;
