...
bool Foam::setEndPointsDisplacement::setBC()
{
	...
	)
	{
	    ...
	    Info << "time" << tab << time_.value() + time_.deltaT().value()
		 << "\ndisp" << tab << curYdispMag
		 << endl;
	...
	else
	{
	    ...
	    // vector curW(0, yDispEnd_, 0);
	    
	    scalar thetaMag =
		(thetaEnd_/(tEnd - 1))*(time_.value() + time_.deltaT().value() - 1);
		
	    Info << "time" << tab << time_.value() + time_.deltaT().value()
		 << "\nangle" << tab << thetaMag << endl;
